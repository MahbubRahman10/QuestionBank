...
class AdminNormalQuestionController extends Controller
{
    ...
    public function mcqaddquestion(Request $request)
    {
        ...
        if($request->file){
            ...
            $data = \Excel::load($upload_success)->get();
            foreach ($data as $key => $value) {
                if ($value->mcq_type != null) {    

                    $question = ['classsubject_id' => '28', 'section_id' => round($value->mcq_type), 'mcq_type' => '2','question_name' => $value->question, 'c1' => $value->c1, 'c2' => $value->c2, 'c3' => $value->c3, 'option_no_1' => $value->option1, 'option_no_2' => $value->option2, 'option_no_3' => $value->option3, 'option_no_4' => $value->option4, 'correct_answer' => $value->answer];
                ...
                // }
                // else{             
                    // $question = ['classsubject_id' => '28', 'section_id' => round($value->mcq_type), 'mcq_type' => '1','question_name' => $value->question, 'option_no_1' => $value->option1, 'option_no_2' => $value->option2, 'option_no_3' => $value->option3, 'option_no_4' => $value->option4, 'correct_answer' => $value->answer];
                    // DB::table('normal_exam_questions')->insert($question);
                // }
            }
            if(!empty($question))
