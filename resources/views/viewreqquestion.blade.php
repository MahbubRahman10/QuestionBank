...
@section('title')
	 @section('titlename')@if ($clipc == 'ntrca') @lang('index.ntrca')  @elseif($clipc == 'dpe') @lang('index.dpe')  @elseif($clipc == 'bank') @lang('index.bank') @endif
@endsection
@section('content')
	...
	<div class="container" id="viewquestion">
		<div class="col-md-12" >
			<div class="col-md-3" id="sections">
				<h3> @if ($clipc == 'ntrca') NTRCA  @elseif($clipc == 'dpe') DPE  @elseif($clipc == 'bank') BANK @endif
				...
				<ul id="qmenu">
					@foreach ($subject as $data)
						<a @if ($data->id == $getsubject->id ) id="sectionactive"   @else href="/recruitment/question/{{ $clipc }}/{{$data->subject_id}}" @endif><li> {{ $data->subject_name }} </li></a>
					@endforeach
				</ul>
			</div>
			<div class="col-md-9" id="question" >
				<h1 style="font-size: 22px;"> @if ($clipc == 'ntrca') @lang('index.ntrca')  @elseif($clipc == 'dpe') @lang('index.dpe')   @elseif($clipc == 'bank') @lang('index.bank') @endif @lang('index.preparation')</h1>
				...
				<a href="" class="loadmore" data-subid="{{ $getsubject->id }}" data-id="{{ $questions->id }}" data-category="{{ $clipc }}" data-index="{{ $i }}">@lang('index.load')</a>
				@endif
				@else
