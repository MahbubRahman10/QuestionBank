...
@section('title')
    @section('titlename')@lang('index.bcs')
@endsection
@section('content')
			...
			<div class="col-md-3" id="sections">
				...
				<ul id="qmenu">
					@foreach ($subject as $data)
						<a @if ($data->id == $getsubject->subject_id ) id="sectionactive"   @else href="/bcs/{{$data->subject_id}}" @endif><li> {{ $data->subject_name }} </li></a>
					@endforeach
				</ul>
			</div>
			<div class="col-md-9" id="question" >
				<h1 style="font-size: 22px;"> @lang('index.bcs') @lang('index.preparation')</h1>
				...
				<a href="" class="loadmore" data-subid="{{ $getsubject->id }}" data-id="{{ $questions->id }}" data-index="{{ $i }}">@lang('index.load')</a>
				@endif
				@else
		...
		$(document).ready(function(){
			$(document).on('click','.loadmore',function(e){
				...
				$.ajax({
					...
					data:{'type':type,'subid':subid,'id':id,'index':index},
					dataType:"JSON",
					success:function(data){
						if(data == "null"){
							$('.loadmore').remove();
