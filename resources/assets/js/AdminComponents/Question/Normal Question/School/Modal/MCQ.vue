	...
	<div>
			...
			<div class="modal-card">
				...
				<section class="modal-card-body">
					...
					<form @submit.prevent="">
						...
						<div v-show="advancedata">
							<div :class="{ 'has-error': errors.has('paragraph') }">
								<label class="control-label" for="question">Paragraph : </label><br>
								<tinymce  v-model="paragraph" name="paragraph"   :plugins="myPlugins" :toolbar1="myToolbar1" :toolbar2="myToolbar2" :other="myOtherOptions"></tinymce>
								<input type="file" name="image" id="upload" class="hidden" onchange="" style="display:none;">
								<span v-show="errors.has('paragraph')">{{ errors.first('paragraph') }}</span>         
							</div>
							<br>

							<div v-show="adddata" v-for="(n, index) in range">
								<br>
								<div class="control" id="questionfilter">
									<div class="select is-fullwidth">
										<select @change="advelectmcqtype(index)" v-model="n.selecttype">
											<option> Normal </option>
											<option> Advance</option>
										</select>
									</div>
								...
								<div :class="{ 'has-error': errors.has('question') }">
									<label class="control-label" for="question">Question : </label><br>
									<tinymce v-validate="'required'" v-model="n.question" :plugins="myPlugins" :toolbar1="myToolbar1" :toolbar2="myToolbar2" :other="myOtherOptions"></tinymce>
									<input type="file" name="image" id="upload" class="hidden" onchange="" style="display:none;">
									<span v-show="errors.has('question')">{{ errors.first('question') }}</span>         
								</div>
								<br>
								...
								<div :class="{ 'has-error': errors.has('question1') }">
									<label class="control-label" for="question1">Question 1 (i) : </label><br>
									<input v-validate="'required'" v-model="n.question1" type="text" name="question1" class="input">
									<span v-show="errors.has('question1')">{{ errors.first('question1') }}</span>         
								</div>
								<br>
								<div :class="{ 'has-error': errors.has('question2') }">
									<label class="control-label" for="question2">Question 2 (ii) : </label><br>
									<input v-validate="'required'" v-model="n.question2" type="text" name="question2" class="input">
									<span v-show="errors.has('question2')">{{ errors.first('question2') }}</span>         
								</div>
								<br>
								<div :class="{ 'has-error': errors.has('question3') }">
									<label class="control-label" for="question3">Question 3 (iii) : </label><br>
									<input v-validate="'required'" v-model="n.question3" type="text" name="question3" class="input">
									<span v-show="errors.has('question3')">{{ errors.first('question3') }}</span>         
								</div>
								<br>
								<h1>Choose the Option : </h1><br>
							...
							<div :class="{ 'has-error': errors.has('option1') }">
								<label class="control-label" for="option1">Option 1 : </label><br>
								<input v-validate="'required'" v-model="n.option1" type="text" name="option1" class="input">
								<span v-show="errors.has('option1')">{{ errors.first('option1') }}</span>         
							</div>
							<br>
							...
							<div :class="{ 'has-error': errors.has('option2') }">
								<label class="control-label" for="option2">Option 2 : </label><br>
								<input v-validate="'required'" v-model="n.option2" type="text" name="option2" class="input">
								<span v-show="errors.has('option2')">{{ errors.first('option2') }}</span>         
							</div>
							<br>
							...
							<div :class="{ 'has-error': errors.has('option3') }">
								<label class="control-label" for="option3">Option 3 : </label><br>
								<input v-validate="'required'" v-model="n.option3" type="text" name="option3" class="input">
								<span v-show="errors.has('option3')">{{ errors.first('option3') }}</span>         
							</div>
							<br>
							...
							<div :class="{ 'has-error': errors.has('option4') }">
								<label class="control-label" for="option4">Option 4 : </label><br>
								<input v-validate="'required'" v-model="n.option4" type="text" name="option4" class="input">
								<span v-show="errors.has('option4')">{{ errors.first('option4') }}</span>         
							</div>
							<br>
							<div :class="{ 'has-error': errors.has('correct') }">
								...
								<div class="control" id="questionfilter">
									<div class="select is-fullwidth">
										<select v-model="n.correctans" class="input">
											<option> option 1 </option>
											<option> option 2</option>
											<option> option 3</option>
											<option> option 4</option>
						...
						<br>
						<button class="button is-primary" v-on:click="addquestion">Add Question</button>
						<br>
					</div>
					</form>
				</section>
		...
		<div class="modal" v-bind:class="activeviewmodal">
			...
			<div class="modal-card">
				...
				<section class="modal-card-body">
					<!-- Content ... -->
					<h2><strong v-html="question">  </strong></h2><br>
	...
	export default{
		...
		methods:{
			selectmcqtype:function() {
				...
				else if(this.selecttype == 'Advance'){
					this.type = 2
				}
				else{
					this.type = 3
					this.data = false
					this.advancedata = true
				}
			},
			advelectmcqtype:function(index) {
				if(this.range[index].selecttype == 'Normal'){
					this.types = 1
				}
				else if(this.range[index].selecttype == 'Advance'){
					this.types = 2
				}
			},
			addquestion:function() {
				this.adddata = true
				this.range.push({ question: '', question1: '', question2: '', question3: '', option1: '', option2: '', option3: '', option4: '', correctans: '' , selecttype: 'Normal' });
			},
			add:function(id){
				...
				if (vm.file == '') {
					if (vm.type == 1 || vm.type == 2) {
						this.$validator.validateAll().then((result) => {
							if (result) {
								axios.post('/*/admin/question/mcq/addquestion', { 
									secid : vm.secid ,
			...
			editvalidateBeforeSubmit() {
				...
				this.$validator.validateAll().then((result) => {
					if (result) {
						axios.post('/*/admin/question/mcq/editquestion', { 
							id : vm.secid ,
							question : vm.question ,
							option1 : vm.option1 ,
							option2 : vm.option2 ,
			...
			deletequestion() {
				var vm = this
				axios.post('/*/admin/question/mcq/deletequestion', { 
					id : vm.secid
				}).then(response => {
					this.activedeletemodal = ''
