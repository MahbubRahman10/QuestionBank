			...
			<div class="columns">
				<div class="column is-one-quarter">
					<nav class="panel" id="panel">
						...
						<p class="panel-tabs">
							<router-link to="" :to="{name:'generalboardjscquestion'}">JSC</router-link>
							<router-link to="" :to="{name:'generalboardsscquestion'}">SSC</router-link>
							<router-link to="" class="is-active">HSC</router-link>
						</p>
						<a class="panel-block" v-for="items in subjects" v-on:click="getsubject(items.id,items.subject_name)">
				...
				<div class="column" >
					<div class="content" style="background:;border:1px solid #209cee;">
						<div class="content-heading">
							<span class=" is-left"> এইচ এস সি {{subject}} বোর্ড প্রশ্ন </span>
...
export default{
	...
	data(){
		return{
			...
			board : this.$route.params.board.replace("-", " "), // Take Board name from route
	...
	methods:{
		...
		getquestion:function(value){
			...
			axios.post('/*/admin/board/getquestion',{
		...
		add:function(){
			...
			var click = child.add(this.activeSubjectId, 3, this.board);
		},
