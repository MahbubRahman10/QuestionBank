<template>
	<div>
		...
		<Modal ref="modal" v-on:add-category="afteraddcategory" v-on:edit-category="aftereditcategory" v-on:delete-category="afterdeletecategory"></Modal>
		
		<div class="main-contents">
			<div class="notifications" > 
				<span v-if="notify">Category {{msg}} Successfuly</span>
			</div>
			<div class="columns">
				<div class="column" >
					<div class="content" style="background:;border:1px solid #209cee;">
						<div class="content-heading">
							<div class="content-heading">
								<span class=" is-left"> Forum Question Category </span>
						...
						<div class="info">
							<div class="tablesection">
								<div class="field has-addons">
									<div class="control">
										<input class="input" type="text" placeholder="Find Category" v-model="searchcategory" @keyup="searchcategorys()">
									</div>
								</div>
							</div>
							<table>
...
export default{
	...
	methods:{
		...
		getcategory:function($state){
			axios.get('/*/admin/forum/getcategory',{				
				params: {
					page: this.categories.length / 20 + 1
				},
			})
			.then( response => {  
				if (response.data.category.data.length != '') {
					this.categories = this.categories.concat(response.data.category.data);
					$state.loaded();
					console.log(this.categories.length)
					if (this.categories.length == response.data.total) {
						$state.complete();
					}
				}
				else{
		...
		afteraddcategory:function(value) {
		...
		edit:function(index){
			var child = this.$refs.modal;
			var click = child.edit(index,this.categories[index]);
		},
		// Show notification after update
		aftereditcategory:function(index,value) {
			this.$set(this.categories, index, value)
			this.notify = true
			this.msg = 'updated'
			var sel = this
			setTimeout(function() { sel.notify = false }, 5000)
		...
		deletes:function(index){
			var child = this.$refs.modal;
			var click = child.delete(index,this.categories[index].id);
		},
		// Show notification after delete
		afterdeletecategory:function(index) {
		...
		searchcategorys:function () {
			axios.post('/*/admin/forum/getsearchcategory',{
				data : this.searchcategory
			})
			.then( response => {  
